...
import {
  ...
  disconnectProtocol,
  connectProtocol,
  getProtocolConfig as getProtoConnectionConfig,
  setProtocolConfig as setProtoConnectionConfig,
} from '@/api/protocol'
  ...
  try {
    ...
    if (protocolList.value.length > 0) {
      const defaultProtocol =
        wasName && protocolList.value.includes(wasName) ? wasName : protocolList.value[0]
      openConfigModal(defaultProtocol)
    }
  } catch (e) {
    notify.error(getErrorMessage(e, '断开失败'))
    confirmOpen.value = false
  } finally {
...
async function openConfigModal(name: string) {
  ...
  try {
    ...
    notify.error(getErrorMessage(e, '获取协议配置失败'))
  } finally {
    configLoading.value = false
  }
}
...
async function onConnect() {
  ...
  try {
    ...
    await setCoreConfig('AutoConnect', autoConnect.value)
      ...
      <div v-else>
        ...
        <v-row>
          <v-col v-for="name in protocolList" :key="name" cols="12" sm="6" md="4">
            <v-card
              ...
              :class="{
                'border-primary': name === currentProtocol?.name && currentProtocol?.isConnected,
              }"
              height="100%"
            >
              ...
              <v-btn v-else variant="text" color="success" block disabled> 当前连接 </v-btn>
            </v-card>
          </v-col>
        </v-row>
        ...
        <v-dialog v-model="confirmOpen" max-width="420" :persistent="confirmLoading">
          <v-card class="pa-4">
            ...
            <v-card-actions>
              <v-spacer />
              <v-btn variant="text" :disabled="confirmLoading" @click="confirmOpen = false"
                >取消</v-btn
              >
              <v-btn
                variant="tonal"
          ...
          <v-card class="pa-4">
            ...
            <v-card-text>
              ...
              <template v-else>
                ...
                <div v-for="(value, key) in configData" :key="key" class="mb-3">
